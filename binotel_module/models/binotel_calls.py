...
class BinotelMethods(models.AbstractModel):
    ...
    @api.model
    def _stats_incoming_calls_for_period(self, period_start: datetime, period_end: datetime, params: dict) -> dict:
        ...
        if answer['status'] == 'error' \
                and answer['message'].find('Requests are too frequent. You can do this request after') > -1:

            timeout = int(answer['message'][57:answer['message'].find(' sec.')])
            time.sleep(timeout)

            answer = self._stats_incoming_calls_for_period(period_start, period_end, params)

        return answer

    @api.model
    def _stats_outgoing_calls_for_period(self, period_start: datetime, period_end: datetime, params: dict) -> dict:
        ...
        if answer['status'] == 'error' \
            ...
            answer = self._stats_outgoing_calls_for_period(period_start, period_end, params)
    ...
    def _exec_method(self, method_name: str, method_params: dict, params: dict) -> dict:
        ...
        api_key = self.env['ir.config_parameter'].sudo().get_param('binotel.api_key', '')
    ...
    def _form_signature(self, method_params, secret):
        ...
        signature = '' + secret + ('[]' if len(method_params) == 0 else json.dumps(method_params).replace(' ', ''))
    ...
    def form_url(self, host: str, method_name: str, params: dict) -> str:
        ...
        return "https://%s/api/%s/%s.%s" % (host, version, method_name, api_format,)
...
class BinotelCalls(models.Model):
    ...
    def _load_calls(self, loading_date: date = None) -> None:
        ...
        incomings = binotel._stats_incoming_calls_for_period(date_start, date_end, params)
        if params['deny']:
            return
        else:
            calls_data_list.append(incomings)

        outgoings = binotel._stats_outgoing_calls_for_period(date_start, date_end, params)
        if params['deny']:
            return
        else:
        ...
        for calls_data in calls_data_list:
            ...
            for call_key in calls:
                ...
                for binotel_user_id in call_history:
                    ...
                    binotel_user_rec = self.env['binotel.users'].sudo().search([('binotel_id', '=', binotel_user_id)], limit=1)
                    ...
                    if call_rec:
                        ...
                        if not call_rec.ensure_one():
                            _logger.warning(_('Several calls with identical ID\'s found; Call ID: %s') % call_id)
                            continue
                        else:
                            call_rec.write([binotel_call_data, ])
                    else:
                        # creating new call record
                        call_rec = model_binotel_calls.create([binotel_call_data, ])
    ...
    def _get_call_history(self, call: dict, params: dict) -> dict:
        ...
        for history_point in call_history_data:
            ...
            call_history[binotel_user_id] = call_history.get(binotel_user_id, 0) + int(call_duration)
...
class BinotelUsers(models.Model):
    ...
    def _load_users(self):
        ...
        for binotel_user_id in users_dict:
            ...
            existing_user_id = self.env['binotel.users'].search([('binotel_id', '=', binotel_id)])
            if not existing_user_id:
                ...
                if phone:
                    ...
                    domain = odoo.osv.expression.OR([domain, email_domain])
                domain = odoo.osv.expression.AND([domain, [('active', 'in', (True, False))]])
